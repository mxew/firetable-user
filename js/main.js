...
firetable.actions = {
    dubtrackImport: function () {
        ...
        for (var e = 0; e < trackarray.length; e++) {
            ...
            if (numbo == firetable.dtImportList.length)
                $("#importDubResults").html(
                    "Import complete! You can now select another file if you'd like to do another!"
                );
    ...
    loggedIn: function (user) {
        ...
        ftapi.lookup.allLists(function (allPlaylists) {
            ...
            $("#listpicker").html(
                '<option value="1">Add/Delete Playlist</option><option value="0">Default Queue</option>'
            );
            for (var key in allPlaylists) {
                if (allPlaylists.hasOwnProperty(key)) {
                    $("#listpicker").append(
                        '<option id="pdopt' + key + '" value="' + key + '">' + allPlaylists[key].name + "</option>"
                    );
                }
            }
    ...
    cardCase: function () {
        ...
        ftapi.lookup.cardCollection(function (data) {
            for (var key in data) {
                ...
                $("#cardsMain").append(
                    '<span id="caseCardSpot' +
                        key +
                        '" class="caseCardSpot"><canvas width="225" height="300" class="caseCard" id="cardMaker' +
                        key +
                        '"></canvas><span role="button" onclick="firetable.actions.giftCard(\'' +
                        key +
                        '\')" class="cardGiftChat">Gift to DJ</span><span role="button" onclick="firetable.actions.chatCard(\'' +
                        key +
                        '\')" class="cardShareChat">Share In Chat</span></span>'
                );
            ...
            if (data.special) {
                if (data.special == "id8") {
                    ...
                    cake.onload = function () {
                        ...
                        eight.src =
                            "https://indiediscotheque.com/robots/" + data.djid + data.djname + ".png?size=110x110";
                    };
        ...
        if (firetable.preview == id) {
            ...
            firetable.movePvBar = setInterval(function () {
                ...
                $("#pvbar" + firetable.preview).css(
                    "background-image",
                    "linear-gradient(90deg, rgba(244, 129, 11, 0.267) " + pcnt + "%, " + pvcolr + " " + pcnt + "%)"
                );
            }, 200);
            if (type == 1) {
                if (firetable.scLoaded) firetable.scwidget.pause();
        ...
        if (song.type == 1) {
            ...
            firetable.actions.scGet("tracks", song.cid, function (tracks) {
                if (tracks.permalink_url) {
                    ...
                    $tags
                        .find(".tagSongLink")
                        .attr("href", "http://howtojointheindiediscothequewaitlist.com/ThisSongIsBroken?thanks=true");
                }
            });
        }
            ...
            function makeRequest(pg) {
                ...
                request.execute(function (response) {
                    ...
                    if (response.nextPageToken) {
                        ...
                        $("#listpicker").append(
                            '<option id="pdopt' + listid + '" value="' + listid + '">' + name + "</option>"
                        );
...
firetable.utilities = {
            ...
            try {
                ...
                for (const [category, emojisArr] of Object.entries(data[0])) {
                    ...
                    for (let i in emojisArr) {
                        ...
                        words +=
                            duhdoymojis[emojisArr[i].emoji] !== undefined
                                ? "," + duhdoymojis[emojisArr[i].emoji].join(",")
                                : "";
                        $("#picker" + catid).append(
                            '<span role="button" class="pickerResult" title="' +
                                emojisArr[i].slug +
    ...
    isChatPrettyMuchAtBottom: function () {
        ...
        firetable.debug && console.log("near bottom?", scrollable, scrolled);
    ...
    updateAtLand: function () {
        firetable.atUsersFiltered = firetable.atUsers
            .filter((user) => user.toLowerCase().startsWith(firetable.atString.toLowerCase()))
            .sort();
        $("#atPicker").html("");
...
firetable.ui = {
        ...
        reader.onload = function (event) {
            try {
                ...
                if (firetable.dtImportList.length) {
                    $("#importDubResults").text(
                        "Ok... import " + firetable.dtImportName + " (" + firetable.dtImportList.length + " tracks)?"
                    );
    ...
    hidePlayerControls: function () {
        $("head").append(
            "<style class='playerControlsHider'>.previewicon { display: none !important; } div#playerControls { display: none !important; } </style>"
        );
    },
        ...
        if (firetable.showImages) {
            ...
            if (hasImage) {
                chatTxt = chatTxt.replace(imageUrlRegex, function (imageUrl) {
                    ...
                    return (
                        '<a class="inlineImgLink" href="' +
                        imageUrl +
                        '" target="_blank" tabindex="-1"><img src="' +
                        imageUrl +
                        '" class="inlineImage" /><span role="button" class="hideImage">&times;</span></a>'
                    );
                });
            }
        }
    ...
    loginEventsInit: function () {
        ...
        $("#createAccountBttn").bind("click", function () {
            ...
            if (!termsAgreedTo) {
                alert(
                    "You must read and agree to the Terms of Service and Privacy Policy before you can create an account."
                );
            } else if (pass != pass2) {
                alert("Those passwords do not match!");
            } else {
    ...
    init: function () {
        ...
        ftapi.events.on("newHistory", function (data) {
            ...
            $histItem
                .find(".histeal")
                .attr("id", "apv" + data.type + data.cid)
                .on("click", function () {
            ...
            if (firetable.song) {
                if (firetable.song.cid == data.cid && data.adamData.track_name) {
                    ...
                    if (data.adamData.last_play) {
                        $("#lastPlay").text(
                            "last " +
                                firetable.utilities.format_date(data.adamData.last_play) +
                                " by " +
                                data.adamData.last_play_dj
                        );
                    } else {
                        $("#lastPlay").text("");
                    }
                    if (data.adamData.first_play) {
                        $("#firstPlay").text(
                            "first " +
                                firetable.utilities.format_date(data.adamData.first_play) +
                                " by " +
                                data.adamData.first_play_dj
                        );
                    } else {
                        $("#firstPlay").text("");
                    }
                    ...
                    if (showPlaycount) {
                        ...
                        $(".npmsg" + data.cid)
                            .last()
                            .html(
                                '<div class="npmsg">DJ <strong>' +
                                    nicename +
                                    "</strong> started playing <strong>" +
                                    data.adamData.track_name +
                                    "</strong> by <strong>" +
                                    data.adamData.artist +
                                    "</strong><br/>This song has been played " +
                                    data.adamData.playcount +
                                    " times.</div>"
                            );
                    } else {
                        $("#playCount").text("");
        ...
        ftapi.events.on("newSong", function (data) {
            ...
            if (firetable.tagUpdate) {
                if (data.cid == firetable.tagUpdate.cid && firetable.tagUpdate.adamData.track_name) {
                    ...
                    if (firetable.tagUpdate.adamData.last_play) {
                        $("#lastPlay").text(
                            "last " +
                                firetable.utilities.format_date(firetable.tagUpdate.adamData.last_play) +
                                " by " +
                                firetable.tagUpdate.adamData.last_play_dj
                        );
                    }
                    if (firetable.tagUpdate.adamData.first_play) {
                        $("#firstPlay").text(
                            "first " +
                                firetable.utilities.format_date(firetable.tagUpdate.adamData.first_play) +
                                " by " +
                                firetable.tagUpdate.adamData.first_play_dj
                        );
                    }
            ...
            if (data.cid != 0) {
                ...
                if (firetable.nonpmsg) {
                    ...
                    if (showPlaycount) {
                        $("#chats").append(
                            '<div class="newChat nowplayn npmsg' +
                                data.cid +
                                '"><div class="npmsg">DJ <strong>' +
                                nicename +
                                "</strong> started playing <strong>" +
                                ...
                                data.artist +
                                "</strong><br/>This song has been played " +
                                firetable.tagUpdate.adamData.playcount +
                                " times.</div>"
                        );
                    } else {
                        $("#chats").append(
        ...
        ftapi.events.on("lightsChanged", function (data) {
            ...
            if (data) {
                ...
                var style =
                    "<style class='festiveLights'>.lightrope { text-align: center; white-space: nowrap; overflow: hidden; position: absolute; z-index: 1; margin: -6px 0 0 0; padding: 0; pointer-events: none; width: 100%; z-index: 55; }ul.lightrope li{position: relative; list-style: none; margin: 0; padding: 0; display: block; width: 6px; height: 14px; border-radius: 50%; margin: 10px; display: inline-block; background: #111;} .lightrope li span { position: relative; animation-fill-mode: both; animation-iteration-count: infinite; list-style: none; margin: 0; padding: 0; display: block; width: 6px; height: 14px; border-radius: 50%; display: inline-block; background: rgba(" +
                    colorThing.r +
                    "," +
                    colorThing.g +
                    "," +
                    colorThing.b +
                    ", 1); box-shadow: 0px 2.3333333333px 12px 1.5px rgba(" +
                    colorThing.r +
                    "," +
                    colorThing.g +
                    "," +
                    colorThing.b +
                    ", 1); animation-name: flash-1; animation-duration: 2s; } .lightrope li:nth-child(2n+1) span { background: rgba(" +
                    colorThing.r +
                    "," +
                    colorThing.g +
                    "," +
                    ...
                    "," +
                    colorThing.b +
                    ", 0.5); animation-name: flash-2; animation-duration: 0.4s; } .lightrope li:nth-child(4n+2) span { background: rgba(" +
                    colorThing.r +
                    "," +
                    colorThing.g +
                    "," +
                    ...
                    "," +
                    colorThing.b +
                    ", 1); } 50% { background: rgba(" +
                    colorThing.r +
                    "," +
                    colorThing.g +
                    "," +
                    colorThing.b +
                    ", 0.4); box-shadow: 0px 2.3333333333px 12px 1.5px rgba(" +
                    colorThing.r +
                    "," +
                    colorThing.g +
                    "," +
                    colorThing.b +
                    ", 0.2); } } @keyframes flash-2 { 0%, 100% { background: rgba(" +
                    colorThing.r +
                    "," +
                    colorThing.g +
                    "," +
                    ...
                    "," +
                    colorThing.b +
                    ", 0.2); } } @keyframes flash-3 { 0%, 100% { background: rgba(" +
                    colorThing.r +
                    "," +
                    colorThing.g +
                    "," +
            ...
            if (data) {
                ...
                for (var key in data) {
                    ...
                    if (data.hasOwnProperty(key)) {
                        ...
                        ok1 +=
                            '<div class="prson"><div class="botson" style="background-image:url(https://indiediscotheque.com/robots/' +
                            data[key].id +
                            "" +
                            data[key].name +
                            '.png?size=110x110);"></div><span class="prsnName">' +
                            countr +
                            ". " +
                            data[key].name +
                            ' <span class="removemeIcon material-icons"> ' +
                            removeMe +
                            " </span></span></div>";
                        countr++;
                    }
        ...
        ftapi.events.on("tableChanged", function (data) {
            ...
            if (data) {
                ...
                for (var key in data) {
                    if (data.hasOwnProperty(key)) {
                        ...
                        ok1 +=
                            '<div id="spt' +
                            countr +
                            '" class="spot"><div class="avtr" id="avtr' +
                            countr +
                            '" style="background-image: url(https://indiediscotheque.com/robots/' +
                            data[key].id +
                            "" +
                            data[key].name +
                            '.png?size=110x110);"></div><div id="djthing' +
                            countr +
                            '" class="djplaque"><div class="djname"><span class="removemeIcon material-icons"> ' +
                            removeMe +
                            " </span> " +
                            data[key].name +
                            '</div><div class="playcount">' +
                            data[key].plays +
                            '/<span id="plimit' +
                            countr +
                            '">' +
                ...
                if (countr < 4) {
                    ok1 +=
                        '<div class="spot empty"><div class="djplaque"><div class="djname">!addme</div><div class="playcount"></div></div></div>';
                    countr++;
                ...
                if (data[key]) {
                    ftapi.lookup.userByName(key, function (person) {
                        $("#activeSuspentions").append(
                            '<div class="importResult"><div class="imtxt">' +
                                person.username +
                                '</div><i role="button" onclick="firetable.actions.unban(\'' +
                                person.userid +
                                '\')" class="material-icons" title="Unsuspend">&#xE5C9;</i></div>'
                        );
                    });
                }
            }
        ...
        ftapi.events.on("userJoined", function (data) {
            ...
            newUserToAddX.html(
                '<div class="botson" style="background-image:url(https://indiediscotheque.com/robots/' +
                    data.userid +
                    "" +
                    data.username +
                    '.png?size=110x110);"><span class="material-icons block">' +
                    blockcon +
                    '</span><span class="material-icons herecon ' +
                    isIdle +
                    '">' +
                    herecon +
                    '</span></div><span class="prsnName">' +
                    data.username +
                    '</span><span class="utitle">' +
                    rolename +
                    '</span><span class="prsnJoined">joined ' +
                    firetable.utilities.format_date(data.joined) +
                    "</span>"
            );
        ...
        $("#plmaker").bind("keyup", function (e) {
            if (e.which == 13) {
                ...
                if (val != "") {
                    ...
                    $("#listpicker").append(
                        '<option id="pdopt' + listid + '" value="' + listid + '">' + val + "</option>"
                    );
        ...
        $("#grab").bind("click", function () {
            ...
            if (isHidden) {
                ftapi.lookup.allLists(function (allPlaylists) {
                    $("#stealpicker").html(
                        '<option value="-1">Where to?</option><option value="0">Default Queue</option>'
                    );
                    for (var key in allPlaylists) {
                        if (allPlaylists.hasOwnProperty(key)) {
                            $("#stealpicker").append(
        ...
        $("#mergeLists").bind("click", function () {
            ...
            if (isHidden) {
                ftapi.lookup.allLists(function (allPlaylists) {
                    ...
                    $("#mergepicker2").html(
                        '<option value="-1">Create New Copy</option><option value="0">Default Queue</option>'
                    );
                    for (var key in allPlaylists) {
                        if (allPlaylists.hasOwnProperty(key)) {
                            $("#mergepicker").append(
                                            ...
                                            if (response.result.items.length === 1) {
                                                ...
                                                $("#importResults").append(
                                                    '<div class="importResult"><div class="imtxt">' +
                                                        ...
                                                        '</div><a target="_blank" href="https://www.youtube.com/playlist?list=' +
                                                        listID +
                                                        '" class="importLinkCheck"><i class="material-icons">&#xE250;</i></a> <i role="button" onclick="firetable.actions.importList(\'' +
                                                        listID +
                                                        "', '" +
                                                        firetable.utilities.htmlEscape(item.snippet.title) +
                                                        '\', 1)" class="material-icons" title="Import">&#xE02E;</i></div>'
                                                );
                                            } else {
                                                // no result
                                            }
                                    ...
                                    if (item.sharing == "public" && item.kind == "playlist") {
                                        $("#importResults").append(
                                            '<div class="importResult"><div class="imtxt">' +
                                                ...
                                                " (" +
                                                item.track_count +
                                                ' songs)</div><a target="_blank" href="' +
                                                item.permalink_url +
                                                '" class="importLinkCheck"><i class="material-icons">&#xE250;</i></a> <i role="button" onclick="firetable.actions.importList(\'' +
                                                item.id +
                                                "', '" +
                                                firetable.utilities.htmlEscape(item.title) +
                                                '\', 2)" class="material-icons" title="Import">&#xE02E;</i></div>'
                                        );
                                    }
                                }
                            });
        ...
        $("#newchat").bind("keypress", function (e) {
            ...
            if (e.key == "Enter") {
                ...
                if (e.key == " " || e.key == "Spacebar") {
                    ...
                    $("#atPicker").html("");
                    $('<div class="atPickerThing"><i>Usernames cannot contain "' + e.key + '"</i></div>').appendTo(
                        "#atPicker"
                    );
                } else {
                    // we're still in @ land
      ...
      */
            ...
            $("head").append(
                "<style class='customColorStyles'>:focus { box-shadow: 0 0 0.5rem " +
                    ...
                    firetable.color +
                    "66; box-shadow: inset 0 0 1rem " +
                    firetable.color +
                    "33; } .simplebar-scrollbar:before { background: " +
                    firetable.color +
                    "; }</style>"
            );
